public class Philosophers {
    private static final int NUMBER_OF_PHILOSOPHERS = 3;
    private static final int THINK_TIME = 100;
    private static final int EAT_TIME = 50;

    private static final Object[] forks = new Object[NUMBER_OF_PHILOSOPHERS];
    static {
    ...
    private static class Philosopher extends Thread {
        ...
        private void eat() {
            int lowId = id;
            int highId = (id + 1) % NUMBER_OF_PHILOSOPHERS;

            System.err.println("#" + id + " Taking left fork.");
            synchronized (forks[lowId]) {
                /*try {
                    Thread.sleep(THINK_TIME);
                } catch (InterruptedException e) {
                    e.printStackTrace();
